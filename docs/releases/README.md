...
flowchart TD
    ...
    PSnap["publish-snap<br/>stable, candidate, beta"]:::publish
...
| `clipman_*.snap` | Snap package | Snap Store auto-publish (stable, candidate, beta); offline install via `snap install --dangerous` |
...
- **`.deb` and `.rpm`** install the daemon, the Python module, the
  ...
  live in `$HOME` and `gsettings`), and nothing starts the daemon at
  login yet: `clipman setup` is planned for that (#323).
- **No AppImage.** The old job never produced a working one and hid its
  failure; #319 tracks shipping a real, self-contained AppImage.
