...
| Refresh project stats | `refresh-stats.yml` | Daily cron (`0 6 * * *`), `workflow_dispatch`, `push` to `main` touching its own paths | Fetches PyPI and GNOME Extensions counts on the runner, rebuilds the self-hosted star-history SVGs and the downloads history, and commits them to the `stats` branch with the built-in `GITHUB_TOKEN`. No pull request, no personal token; `main` is never touched. | No |
